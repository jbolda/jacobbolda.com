...
Now typing out the full path is a bit of a chore, so using `$PWD` and running thing command from the folder in question, we can shorten it to the following.
