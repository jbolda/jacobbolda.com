---
title: Setting U CI/CD for Tauri
written: "2020-X-x"
path: "/setting-up-ci-cd-for-tauri/"
category: "CI/CD"
keywords: ["GitHub Actions"]
description: "I've started helping out with a project called Tauri. It will be useful for my personal use, but there's value in having this out in the world even if I end up not using it. When I started looking into it, they were pushing towards releasing an alpha. There was no continuous integration or continuous delivery currently set up, and I have seen the value that it can have in creating focus in a project. Thus began my involvement and we decided to use the shiny new GitHub Actions to deliver this."
---

# Setting Up CI/CD for Tauri
## Background

I've started helping out with a project called Tauri. It will be useful for my personal use, but there's value in having this out in the world even if I end up not using it. When I started looking into it, they were pushing towards releasing an alpha. There was no continuous integration or continuous delivery currently set up, and I have seen the value that it can have in creating focus in a project. Thus began my involvement and we decided to use the shiny new GitHub Actions to deliver this. It was still on beta, but it had gone through iterations over months and months that we felt it was stable enough to use (and nearly out of beta anyway). The deep integration with GitHub was and will be incredibly useful for us.

We didn't have much in the way of tests to run as much of the codebase had been more recently pulled out of other projects. The quickest win was to take and create some example projects and use those as what we're calling smoke tests. We took the manual process of testing in projects, and dumped that into CI. They are effectively "integration" tests where we will take any HTML, CSS and JS and build it with Tauri.

## Next Phase of Testing

This served us well and was able to catch a few bugs in the process. We had the focus to get to and meet our release goal of launching the first alpha build. As the confidence in our code base has risen, the time these smoke tests take to run compared to the value delivered has flipped. The time that it takes to run the smoke tests has become painful. Originally we had these tests run on every push to a PR, as we wanted a tight feedback loop in fixing any issues we had. Now that we are starting to add more unit tests, this may not be something that needs to run on every push to get the feedback needed. The next phase in our setup will be running our unit tests on every push, and dialing back our smoke test runs.

Github Actions has two triggers of which we make heavy use: `push` and `pull_request`. Every commit that is made to the repo is considered a `push`. When you open a pull request from a branch (say call it `great_feature`) to another branch (say our working branch, `dev`), each commit to `great_feature` would possibly trigger both of these events. We can use a filter to focus on the events we care about though. In our workflows, we only PR (pull request) the `dev` and `master` branches. This means that if we filter to only the `dev` and `master` branches on commit, we will only run that workflow when we _merge_ a PR. A merged PR only occurs once a day or less typically so this will be a good fit for the longer running tests, e.g. the smoke tests in our case. Below is how that might look.
...
Let us touch on this for a moment. The smoke tests are a handful of examples created using the major frameworks (Vue, create-react-app, Quasar, Next.js, Gatsby, ...). Originally these resided in a separate repository. They were then moved into the main Tauri repo for what seemed worthwhile benefits. We implemented the Renovate bot which opens a pull request to upgrade dependencies including for our examples. Remembering our previously mentioned Github Actions, this would trigger our tests to run. It was nice to see these tested every time our examples updated. Having the examples close by and wired up also made for testing locally much easier. The downside is that all of these pull requests created a lot of noise. The examples were flooding our CI and commit history.
...
The version bump and polishing sequences are tightly coupled to JavaScript though. However, the implementation of `changesets` is split up nicely into packages each with their own scope. We can potentially use just the packages that parse the markdown files into version changes, and build a life cycle that version bumps and publishes based on a script that we pass. There is nothing that is yet published, but experimentation has shown promising results. Regardless if the code changing is Rust or JavaScript, we can describe the change in markdown. This will parse the markdown for us and we can then issue commands based on the described version bump. After the version is bumped, we need only issue a publish command for crates or npm. Since we don't need complete dynamic use of this library, we can just manually set each package to publish using a specified script.
...
Later phases of our Github Actions implementation can even look to do more on top of a typical publish workflow. This project has a deep consideration for security that is worth bringing to our publishing workflow. A potential workflow could involve publishing the package to multiple package managers including a private one hosted by Tauri. We can also do some interesting things like signing our releases, including a hash in the release and/or even publishing this information on a blockchain that can be easily verified.
